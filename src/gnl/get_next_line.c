...
static char	*init_stash(char **stash)
{
	...
	if (!(*stash))
	{
		*stash = ft_calloc(GNL_BUFSIZ + 1, 1);
		if (!(*stash))
			return (NULL);
		return (*stash);
	}
	...
	new_stash = ft_strjoin("", *stash + lb_pos + 1, -1, false);
	free(*stash);
	return (new_stash);
}

static bool	init(int fd, char **stash, char **buffer, int *bytes_read)
{
	int	i;

	if (fd < 0 || GNL_BUFSIZ <= 0)
		return (false);
...
static char	*handle_lb(char **stash, char *buffer, int lb_pos, int bytes_read)
{
	char	*result;
	int		stash_len;
...
char	*get_next_line(int fd)
{
	...
	if (!(*buffer))
		bytes_read = read(fd, buffer, GNL_BUFSIZ);
	if (bytes_read == -1)
		return (free(stash), free(buffer), stash = NULL, NULL);
	lb_pos = ft_strfind(buffer, '\n', 0, 0);
	while (bytes_read == GNL_BUFSIZ && lb_pos == -1)
	{
		stash = ft_strjoin(stash, buffer, -1, true);
		if (!stash)
			return (free(buffer), NULL);
		bytes_read = read(fd, buffer, GNL_BUFSIZ);
		if (bytes_read == -1)
